...
// 不使用 DOM（除了觸控按鈕），所有元素都畫在同一個 ctx 上。
...
export class DialogueBox {
  ...
  draw(ctx, viewW, viewH) {
    ...
    const visText = this.current.text.slice(0, this.chars);
    wrapText(ctx, visText, x + 44, y + 28, w - 54, 10, C.text);
...
// ── 任務追蹤 HUD（右上）──────────────────────────────────────────────────────
export function drawMissionHUD(ctx, completed, total, rescued, rescueTotal, objective, viewW) {
  const w = 140, x = viewW - w - 6, y = 6, h = 40;
  panel(ctx, x, y, w, h, '#0a0f24', '#505878');

  ctx.fillStyle = C.glow_gold;
  ctx.font = 'bold 7px monospace';
  ctx.fillText('任務', x + 10, y + 14);

  ctx.fillStyle = C.text;
  ctx.font = FONT_SM;
  if (objective < 2) {
    ctx.fillText(`修復節點  ${completed} / ${total}`, x + 10, y + 27);
  } else {
    const col = rescued >= rescueTotal ? '#60e090' : C.text;
    ctx.fillStyle = col;
    ctx.fillText(`救出居民  ${rescued} / ${rescueTotal}`, x + 10, y + 27);
  }

  // 進度條
  const barW = w - 20;
  const progress = objective < 2 ? completed / total : rescued / rescueTotal;
  ctx.fillStyle = '#202438';
  ctx.fillRect(x + 10, y + 32, barW, 3);
  ctx.fillStyle = progress >= 1 ? '#60e090' : C.glow_gold;
  ctx.fillRect(x + 10, y + 32, Math.round(barW * progress), 3);
}
